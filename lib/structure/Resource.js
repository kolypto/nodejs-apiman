 ...
 *      Each function must return a promise which determines that the function has finished.
 * @returns {Resource}
 */
...
/** Add an endpoint method: the method that handles all requests that fall into the resource.
...
/** Add methods from a Controller.
 * ApiMan only imports methods, and only those that does not start with an underscore.
 * To set middleware methods, put them in the `middleware` property of the method function.
 * @param {Object} ctrl
 *      The controller to add the methods from.
 * @returns {Resource}
 */
Resource.prototype.controllerMethods = function(ctrl){
    // Arguments
    if (!_.isObject(ctrl))
        throw new Error('apiman.Resource#controllerMethods: ctrl must be an object');

    // Import
    var verb, method, middleware;
    for (verb in ctrl){ // iterate all props through the prototype chain
        method = ctrl[verb];

        // Skip non-functions and protected functions
        if (!_.isFunction(method) || verb[0] === '_')
            continue;

        // Middleware?
        middleware = [];
        if (!_.isUndefined(method.middleware))
            middleware = [].concat(method.middleware);

        // Import
        this.method.apply(this,
            [verb]
            .concat(middleware)
            .concat(method.bind(ctrl))
        );
    }
 ...
 *      After the request, this object receives additional Request fields like sessionID and other.
 * @returns {Q} promise for an object { req: Request, res: * }
 *
...
Resource.prototype.exec = function(path, verb, args, req){
    // Arguments
    var req_provided = !_.isUndefined(req);
    args = args || {};
    req = req || {};
    if (!_.isString(path))
    ...
    // Return a promise
    return response
        // Merge the *system* and *result* channels
        .getResult()
