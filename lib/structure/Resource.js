 ...
 * Using this name, you can reference it later
 * @param {String} name
 * @returns {exports}
 */
...
/** Get a resource by name hierarchy
 * @param {Array.<String>} names
 *      Array of resource names to descend into the hierarchy
 * @returns {Resource?}
 */
Resource.prototype.getResource = function(names){
    var res = this;
    _.each(names, function(name){
        if (res === undefined) return;
        res = _.findWhere(res.resources, { name: name });
    });
    return res;
};

/** Get the map of all named resources
 * @returns {Object}
 */
Resource.prototype.getResourcesMap = function(){
    var gen = function(resource, map){
        _.each(resource.resources, function(resource){
            if (resource.name === undefined)
                return;
            map[resource.name] = {};
            gen(resource, map[resource.name]);
        });
        return map;
    };
    return gen(this, {});
...
/** Create an express-style resource with :named parameters.
 * This assumes you use '/' for resource separation.
 * @param {String} path
 *      Express-style path with parameters:
 *      - Use :name for required parameters
 *      - Use :name? for optional parameters
 * @param {Object.<String, { regex: RegExp, proc: function(req, value) }>?} opts
 *      Overrides for the default param behavior. Optional.
 * @returns {Resource}
 */
Resource.prototype.xresource = function(path, opts){
    var self = this;
    opts = opts || {};

    // Prepare the RegExp
    var params = [];
    path = new RegExp('^' + path.replace(/(\/?):([a-z0-9_]+)([\?]?)/ig, function(match, sws, name, opt, pos){
        // Prepare the grabber regexp
        params.push(name);
        var regex;
        if (opts[name] && opts[name].regex)
            regex = opts[name].regex;
        else
            regex = '[^/]+';
        regex = '(' + regex + ')';
        ...
        // Required param
        if (opt === '')
            return sws + regex; // return as is, don't forget the prefix

        // Optional param: can grab the preceding '/'
        if (opt.indexOf('?') !== -1){
            return sws + '?' + '(?:' + regex + '|$)';
        }
    ...
    // Create the resource
    var r = this.resource(path);

    // Map params
    _.each(params, function(name, i){
        var proc;
        if (opts[name] && opts[name].proc)
            proc = opts[name].proc;
        else
            proc = function(req,value){ return value; };

        r.param(i+1, (function(name, proc){
            if (proc.length === 2)
                return function(req, value){
                    req.params[name] = proc(req, value);
                };
            else // Middleware
                return proc;
        })(name, proc));
 ...
 * @param {String|function(req: Request, res: Response, next: function(Error?), value:*):*?} callback
...
Resource.prototype.param = function(index, callback){
    ...
    if (_.isFunction(callback) && !( callback.length == 2 || callback.length == 4 ))
        throw new Error('apiman.Resource#param: `callback` arity must be 2 or 4');
    // Proceed
    ...
    if (_.isRegExp(this.path)){
        ...
        if (request){
            ...
            for (var i = 1; i<this.params.length; i++)
                (function(param, val, i){
                    // Defined params
                    if (param !== undefined){
                        // Named params
                        if (!_.isFunction(param))
                            request.params[param] = val;
                        else if (param.length == 2) // Simple Param: ( req, val )
                            param(request, val);
                        else // Middleware Param: (req, res, next, val)
...
Resource.prototype.which = function(path, verb, request){
    ...
    if (request && this.mappers[request.from] !== undefined){
        var m = this.mappers[request.from](request, match.tail, verb, match.head);
        if (m){
            path = m[0];
...
Resource.prototype.requestMethod = function(request, callback){
    ...
    async.series(
        _(request.middleware).map(function(mw){
            return _(mw).partial(request, response).value(); // cb(req, res,   next), `next(err)` provided by async
        }).value(),
        function(err){
            if (err)
                response.error(err);
...
Resource.prototype.map = function(from, mapper){
    ...
    if (!_.isFunction(mapper)){
        ...
        var f = function(req, path, verb, match){
